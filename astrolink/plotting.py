...
def plotAggregationTree(c, ax = None):
    """
    ...
    offSet = 0.02
    yPos_leq = c.groups[:, 2] - 1
    yPos_geq = c.groups[:, 1] - 1
    heights = np.column_stack((logRhoOrdered[yPos_leq], logRhoOrdered[yPos_geq])).mean(axis = 1) - offSet 

    # Span
    ax.hlines(heights, c.groups[:, 1], yPos_leq, colors = 'red', linewidth = 0.8, zorder = 3)
    ax.hlines(heights, c.groups[:, 0], yPos_geq, colors = 'royalblue', linewidth = 0.8, zorder = 2)
    
    # Ends
    for i in range(2):
        xPos_leq = c.groups[:, i + 1] if i == 0 else c.groups[:, i + 1] - 1
        ax.vlines(xPos_leq, heights, heights + 0.75*offSet, colors = 'red', linewidth = 0.8, zorder = 3)
        xPos_geq = c.groups[:, i] if i == 0 else c.groups[:, i] - 1
        ax.vlines(xPos_geq, heights, heights + 0.75*offSet, colors = 'royalblue', linewidth = 0.8, zorder = 2)
    
    # Joins
    for i, group in enumerate(c.groups):
        parent_leq = next((i - j - 1 for j, g in enumerate(c.groups[:i][::-1]) if group[1] > g[1] and group[2] <= g[2]), None)

        possible_parents = np.where(np.logical_and(c.groups[:, 0] <= group[0], c.groups[:, 1] > group[1]))[0]
        if possible_parents.size:
            parent_geq = possible_parents[np.diff(c.groups[possible_parents, :2], axis = -1).argmin()]
            yLower_geq = heights[parent_geq]
        else: yLower_geq = 0
        parent_geq = next((i - j - 1 for j, g in enumerate(c.groups[:i][::-1]) if group[0] >= g[0] and group[1] <= g[1]), None)
...
def plotSignificanceModel(c, ax = None):
    """
    ...
    finiteSigs = c.groups_sigs[np.isfinite(c.groups_sigs[:, 1]), 1]
    ...
    # Plot cutoff over prominences histogram
    lineCollection = ax.vlines(c.S, 0, h.max(), color = 'royalblue', ls = 'dashed', lw = 1)
